...
```bash
REPO_CWD=$(pwd) && cd <plugin_root>/scripts && uv run python -m scripts stale 2>/dev/null
```
...
```bash
REPO_CWD=$(pwd) && cd <plugin_root>/scripts && uv run python -m scripts migrate --from <path>
```
